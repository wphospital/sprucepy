...
def create_task2(name, path, script_args, start_in, python_path=None, task_trigger=None, start=None, duration=None, repetition=None):

    def get_python_path():
        """
        Get the path for python on the machine.
        Take the first in the list
        """
        stream = os.popen('where python')
        output = stream.read()
        # read the command line outputs into a list
        return output.split('\n')[0]
    ...
    try:
        ...
        if repetition is not None:
            repetition_interval = 'PT' + str(repetition)
        else:
            repetition_interval = ''
        ...
        # task trigger
        if task_trigger:
            TASK_TRIGGER_TIME = trigger_types.get(
                task_trigger)  # the the task trigger if valid
        if task_trigger is None:
            TASK_TRIGGER_TIME = 2  # default daily
        ...
        #action.Arguments = 'C:/Users/miz9013/git/NYPAnalyticsToolkit/hello.py'
        action.Arguments = task
        action.WorkingDirectory = start_in

        # Add Action
        #
        # Add Action
        #add_action(task_definition=name, action_type='Execute', start_in=start_in)
        ...
        # TODO: notify Spruce API of the schedule

        return True
    except:
        # TODO notify Spruce API of the failure
...
def info(name, location="\\"):
    ...
    for actionObj in task.Definition.Actions:
        action = {"action_type": _reverse_lookup(
            action_types, actionObj.Type)}
        if actionObj.Path:
    ...
    for triggerObj in task.Definition.Triggers:
        trigger = {"trigger_type": _reverse_lookup(
            trigger_types, triggerObj.Type)}
        ...
        if hasattr(triggerObj, "Delay"):
            if triggerObj.Delay:
                trigger["delay"] = _reverse_lookup(
                    duration, triggerObj.Delay)
            else:
                trigger["delay"] = False
...
def add_trigger(
    ...
    *OnSessionChange*
    ...
    if kwargs.get("task_definition", False):
        ...
        if repeat_interval:
            trigger.Repetition.Interval = _lookup_first(
        ...
        if execution_time_limit:
            trigger.ExecutionTimeLimit = _lookup_first(
                duration, execution_time_limit)
        if end_boundary:
        ...
        # Monthly Trigger Parameters
            ...
            if kwargs.get("days_of_month", False) or kwargs.get(
                ...
                trigger.RunOnLastDayOfMonth = kwargs.get(
                    "last_day_of_month", False)
            else:
                return (
        ...
        # Monthly Day Of Week Trigger Parameters
            ...
            if kwargs.get("weeks_of_month", False) or kwargs.get(
                ...
                trigger.RunOnLastWeekOfMonth = kwargs.get(
                    "last_week_of_month", False)
            else:
                return (
